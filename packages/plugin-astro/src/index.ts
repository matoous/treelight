  ...
  return {
    hooks: {
      'astro:config:setup': ({ config, logger, updateConfig }) => {
        ...
        if (processor) {
          logger.warn(
            '@treelight/plugin-astro could not install @treelight/plugin-rehype because markdown.processor is not a supported satteri or unified processor. Add @treelight/plugin-rehype to your Markdown processor manually.',
          );
          return;
        }
